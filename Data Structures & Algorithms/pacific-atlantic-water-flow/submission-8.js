class Solution {
    ...
    pacificAtlantic(heights) {
        ...
        function flowToPacific(x, y) {
            if (heights[y][x] >= heights[y][x - 1]) {
                willFlowToPacific[y][x] ||= willFlowToPacific[y][x - 1];
            }
            if (heights[y][x] >= heights[y - 1][x]) {
                willFlowToPacific[y][x] ||= willFlowToPacific[y - 1][x];
            }
        }
        function flowToAtlantic(x, y) {
            if (heights[y][x] >= heights[y][x + 1]) {
                willFlowToAtlantic[y][x] ||= willFlowToAtlantic[y][x + 1];
            }
            if (heights[y][x] >= heights[y + 1][x]) {
                willFlowToAtlantic[y][x] ||= willFlowToAtlantic[y + 1][x];
            }
        }
        ...
        //for (let y = 1; y < islandWidth; y++) {
        //    for (let x = 1; x < islandLength; x++) {
        //        flowToPacific();
        //    }
        //}
        //for (let y = islandWidth - 2; y >= 0 ; y--) {
        //    for (let x = islandLength - 2; x >= 0 ; x--) {
        //        flowToAtlantic();
        //    }
        //}
        for (let y = 0; y < islandWidth; y++) {
